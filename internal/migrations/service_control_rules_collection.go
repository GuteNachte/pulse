...
func init() {
		...
		jsonData := `[
	{
		...
		"fields": [
			...
			{"autogeneratePattern":"","hidden":false,"id":"text3051925876","max":0,"min":0,"name":"note","pattern":"","presentable":false,"primaryKey":false,"required":false,"system":false,"type":"text"},
		...
		"indexes": [
			"CREATE UNIQUE INDEX ` + "`" + `idx_service_control_rules_unique` + "`" + ` ON ` + "`" + `service_control_rules` + "`" + ` (` + "`" + `system` + "`" + `, ` + "`" + `platform` + "`" + `, ` + "`" + `name` + "`" + `)"
		]
	}
]`
