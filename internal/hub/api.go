...
func (h *Hub) registerApiRouteGroup(se *core.ServeEvent, prefix string) error {
	...
	apiAuth.GET("/systems/summary", h.listSystemSummaries)
	...
	apiAuth.GET("/website-monitors", h.listWebsiteMonitors)
	// get container system summaries and the selected system's container list
	apiAuth.GET("/containers", h.listContainers)
	...
	apiAuth.POST("/agent-releases/sync", h.syncAgentReleases).BindFunc(excludeReadOnlyRole)
	apiAuth.POST("/pairing-codes", h.createPairingCode).BindFunc(excludeReadOnlyRole)
	apiAuth.GET("/pairing-codes", h.listPairingCodes).BindFunc(excludeReadOnlyRole)
	apiAuth.GET("/pairing-codes/{id}", h.getPairingCode).BindFunc(excludeReadOnlyRole)
	apiAuth.GET("/agent-tokens", h.listAgentTokens).BindFunc(excludeReadOnlyRole)
	apiAuth.GET("/agent-tokens/system/{id}/secret", h.getAgentTokenSecretBySystem).BindFunc(excludeReadOnlyRole)
	apiAuth.GET("/agent-tokens/{id}/secret", h.getAgentTokenSecret).BindFunc(excludeReadOnlyRole)
	apiAuth.POST("/agent-tokens/{id}/rotate", h.rotateAgentToken).BindFunc(excludeReadOnlyRole)
	apiAuth.POST("/agent-tokens/{id}/unbind", h.unbindAgentToken).BindFunc(excludeReadOnlyRole)
	...
	apiAuth.GET("/alert-policies", alerts.ListGlobalAlertPolicies)
	apiAuth.POST("/alert-policies", alerts.UpsertGlobalAlertPolicy).BindFunc(excludeReadOnlyRole)
	apiAuth.DELETE("/alert-policies", alerts.DeleteGlobalAlertPolicy).BindFunc(excludeReadOnlyRole)
	apiAuth.GET("/alerts-history", alerts.ListAlertHistory)
	apiAuth.POST("/alerts-history/{id}/acknowledge", alerts.AcknowledgeAlertHistory).BindFunc(excludeReadOnlyRole)
	apiAuth.POST("/alerts-history/{id}/silence", alerts.SilenceAlertHistory).BindFunc(excludeReadOnlyRole)
	apiAuth.POST("/alerts-history/{id}/unsilence", alerts.UnsilenceAlertHistory).BindFunc(excludeReadOnlyRole)
	// refresh SMART devices for a system
	apiAuth.POST("/smart/refresh", h.refreshSmartData).BindFunc(excludeReadOnlyRole)
	apiAuth.GET("/services/search", h.searchServices).BindFunc(excludeReadOnlyRole)
	apiAuth.GET("/software/search", h.searchSoftware).BindFunc(excludeReadOnlyRole)
	...
	apiAuth.GET("/logs", h.getSystemLogs)
	...
	apiAuth.GET("/backups", h.listBackups).BindFunc(requireAdminRole)
	apiAuth.POST("/backups", h.createBackup).BindFunc(requireAdminRole)
	apiAuth.GET("/backups/{key}", h.downloadBackup).BindFunc(requireAdminRole)
	apiAuth.DELETE("/backups/{key}", h.deleteBackup).BindFunc(requireAdminRole)
	apiAuth.POST("/backups/{key}/restore", h.restoreBackup).BindFunc(requireAdminRole)
	apiAuth.POST("/website-monitors/{id}/check", h.checkWebsiteMonitorNow).BindFunc(excludeReadOnlyRole)
	// /containers routes
