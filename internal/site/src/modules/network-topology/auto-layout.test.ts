...
test("creates stable suggested positions without mutating graph records or endpoints", () => {
	...
	assert.ok(
		Math.abs(first.nodes[assetNodeId(router.id)].x - first.nodes[assetNodeId(internet.id)].x) >=
			SUGGESTED_NODE_WIDTH
	)
	assert.ok(
		Math.abs(first.nodes[assetNodeId(switchAsset.id)].x - first.nodes[assetNodeId(router.id)].x) >=
			SUGGESTED_NODE_WIDTH
	)
