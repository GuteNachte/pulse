...
从 `1.0.6` 起，Pulse 后续按“基础底座 + 可管理功能模块”演进。基础底座包含 PocketBase 基座和 Pulse 内核；认证、权限、数据迁移、模块注册表和模块管理属于不可关闭底座能力。客户端资产、机器详情、容器、网站、告警、Agent 接入、S.M.A.R.T.、网络拓扑和设置子功能都要逐步成为可观测、可开关、边界清楚的功能模块。新增功能必须先按 `docs\module-architecture.md` 确认模块归属、目录、manifest、开关语义和设置页状态展示，避免项目变大后形成互相缠绕的功能堆。
