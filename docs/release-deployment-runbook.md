...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File supplemental\scripts\verify-release-v1.ps1 -Version 1.0.5 -HubUrl http://127.0.0.1:8090
```
...
- Android APK metadata 的 `versionName` 是 `1.0.5`，`versionCode` 是 `10005`。
- Harbor 上 `pulse-hub:1.0.5` 和 `pulse-agent:1.0.5` 可被 `docker manifest inspect` 读取。
- 可选 Hub URL 返回 `/api/health` 正常，`/api/pulse/public-info` 版本为 `1.0.5`。
...
- `/api/health` 返回成功。
- `/api/pulse/public-info` 的 `v` 是 `1.0.5`。
- `pulse-hub` 镜像是 `registry.example.com/infra/pulse-hub:1.0.5`。
- `pulse-agent` 镜像是 `registry.example.com/infra/pulse-agent:1.0.5`。
- 两个容器都是 `host` 网络。
- 页面 About 显示 Hub / Web / Android / Agent 目标版本为 `1.0.5`。
- Hub 所在机器在线，显示真实机器名并带 `Hub` 标签。

## 6. 回滚

如果 `1.0.5` 部署后健康检查、登录、Agent 连接或核心页面异常，先回滚到上一稳定版本 `1.0.4`。
...
sed -i 's#registry.example.com/infra/pulse-hub:1.0.5#registry.example.com/infra/pulse-hub:1.0.4#g' docker-compose.yml
sed -i 's#registry.example.com/infra/pulse-agent:1.0.5#registry.example.com/infra/pulse-agent:1.0.4#g' docker-compose.yml
```
