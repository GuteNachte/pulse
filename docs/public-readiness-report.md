...
- 两类已登记私有基础设施端点已从 `HEAD` 可达历史中完成定向替换，精确历史扫描为 0 条发现；业务文件和提交顺序未删除。
- 审计契约覆盖禁止路径、允许占位符、源码端点、当前分支历史端点、旁支隔离、Gitleaks `HEAD` 参数、扫描器输出隔离和政策文件。
...
- 实际文件内容受替换的 6 个提交映射如下；其余后代提交仅因父提交变化而产生新的哈希。
